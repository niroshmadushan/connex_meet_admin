  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get('http://192.168.13.150:3001/getallspecialbookingssessions', { withCredentials: true });
    ...
    <Box sx={{ padding: 3, backgroundColor: '#f9fafb', minHeight: '100vh' }}>
      ...
      {selectedMeeting && (
        <Modal open={open} onClose={handleClose} sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center',overflow:'scroll',mt:2 }}>
          <Fade in={open}>
            <Paper sx={{ padding: 4, width: '80%', maxWidth: '600px',height:'60vh',overflowY:'scroll' }}>
